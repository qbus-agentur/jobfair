...
class JobController extends ActionController {
	...
	*/
	...
	public function createAction(Job $newJob) {
		...
		if ($this->settings['new']['enableAdminNotificaton'] &&
			...
			// from
            $sender = new Address($this->settings['new']['fromEmail'], $this->settings['new']['fromName']);
			// to
			...
			foreach ($to as $pair) {
				if (GeneralUtility::validEmail($pair['email'])) {
					if (trim($pair['name'])) {
						$recipients[] = new Address($pair['email'], $pair['name']);
					} else {
						$recipients[] = new Address($pair['email']);
					}
				}
			}
	...
	*/
	public function createApplicationAction(Application $newApplication, Job $job) {
		...
		if (!empty($attachmentFile['name']) && (!Div::checkExtension($attachmentFile['name']) || !GeneralUtility::makeInstance(FileNameValidator::class)->isValid($attachmentFile['name']))) {
		...
		if ($this->settings['application']['fromEmail']) {
            $sender = new Address($this->settings['application']['fromEmail'], $this->settings['application']['fromName']);
		}
		...
		if ($contact) {
			...
			foreach ($toBcc as $pair) {
				if (GeneralUtility::validEmail($pair['email'])) {
					if (trim($pair['name'])) {
						$recipientsBcc[] = new Address($pair['email'], $pair['name']);
					} else {
						$recipientsBcc[] = new Address($pair['email']);
					}
				}
			}
		}
